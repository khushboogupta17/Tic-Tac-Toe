...
Main logic resides in Gamemanager.cs which controls various states and swapping between them.Player and CPU are taken as scriptable objects so that if we want to have their data in other scenes like shop we can easily access it. It implemented very basic version of FSM where we have different states of game PlayerPlaying,CheckingCondition and GameEnded and switch between them to smotthly transition through game.
